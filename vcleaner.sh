...
sudo aptitude purge $(dpkg -l|awk '{print $2}'|grep -E "linux-(image|headers|debian-modules|restricted-modules)" |grep -vE "linux-(image|headers|restricted-modules)-(generic|i386|server|common|rt|xen)"|grep -v $(uname -r|sed 's/-*[a-z]//g'|sed 's/-386//g'))
